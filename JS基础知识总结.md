...
```
定时器设定一个时间，是最小等待时间，需要同步任务都执行完了才会执行
```
...
```
1、浏览器是多线程,JS是单线程（浏览器只给JS分配一个线程）
单线程：一次只能处理一件事情

2、JS在单线程中实现异步的机制，依赖于浏览器的任务队列。浏览器有两个任务队列（主任务队列，等待任务队列）
```

## 6.2Promise

ES6中新增加的类，目的为了管理JS中的异步编程

三状态：pendding等待态，fulfilled成功态，reject失败态

Promise本身是同步的，可以管理异步操作

new Promise的时候，会立即执行函数体中的异步操作